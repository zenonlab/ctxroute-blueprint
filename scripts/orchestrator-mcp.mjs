...
import { contextQuery, mutateCoordination, prepareMission, readCoordination, reconcileWorktrees, rollbackMission, submitWorkerReport } from './orchestrator-service.mjs';

export const ORCHESTRATOR_TOOL_NAMES = Object.freeze(['ctxroute_context_query', 'orchestrator_read', 'orchestrator_mutate', 'orchestrator_prepare_mission', 'orchestrator_submit_worker_report', 'orchestrator_reconcile_worktrees', 'orchestrator_rollback_mission']);
...
export function createOrchestratorServer(root = process.cwd()) {
  ...
  server.registerTool('orchestrator_reconcile_worktrees', { description: 'Classify managed worktrees and remove only terminal clean allocations.', inputSchema: transactionSchema }, async input => response(await reconcileWorktrees(input, root)));
  server.registerTool('orchestrator_rollback_mission', { description: 'Capture bounded recovery evidence before removing a mission worktree.', inputSchema: transactionSchema }, async input => response(await rollbackMission(input, root)));
