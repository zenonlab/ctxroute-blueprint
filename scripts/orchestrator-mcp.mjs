...
import { commitMission, contextQuery, explainExecution, integrateMission, listOperatingModes, mutateCoordination, pendingDecisions, prepareMission, promoteExperiment, readCoordination, resolveDecision, setOperatingMode, submitWorkerReport } from './orchestrator-service.mjs';

export const ORCHESTRATOR_TOOL_NAMES = Object.freeze(['ctxroute_context_query', 'orchestrator_health', 'orchestrator_read', 'orchestrator_run_goal', 'orchestrator_mutate', 'orchestrator_prepare_mission', 'orchestrator_submit_worker_report', 'orchestrator_commit_mission', 'orchestrator_integrate_mission', 'orchestrator_modes', 'orchestrator_set_mode', 'orchestrator_explain_execution', 'orchestrator_pending_decisions', 'orchestrator_resolve_decision', 'orchestrator_promote_experiment']);
...
export function createOrchestratorServer(root = process.cwd()) {
  ...
  server.registerTool('orchestrator_run_goal', { description: 'Launch assigned local missions through a portable CLI worker adapter.', inputSchema: z.object({ goal_id: z.string(), adapter: z.enum(['codex', 'claude', 'opencode']).optional() }) }, async input => response(await runGoalMissions(input.goal_id, input.adapter ?? 'codex', root)));
  ...
  server.registerTool('orchestrator_commit_mission', { description: 'Create the orchestrator-owned commit for a completed and validated write mission.', inputSchema: transactionSchema }, async input => response(await commitMission(input, root)));
  server.registerTool('orchestrator_integrate_mission', { description: 'Integrate an exact orchestrator-owned commit into the clean primary checkout.', inputSchema: transactionSchema }, async input => response(await integrateMission(input, root)));
