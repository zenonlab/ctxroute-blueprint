...
try {
  ...
  else if (command === 'claim') console.log(JSON.stringify(await claimProgressTicket(file, arg), null, 2));
  ...
  else if (command === 'mode') console.log(JSON.stringify({ ok: true, progress: await setProgressMode(file, arg, false) }, null, 2));
  ...
  else throw new Error('usage: read | status | next <goal-id> | claim <agent-id> [goal-id] | update <update.json> | mode <goal-id> <automatic|manual> | validate-plan <plan.json> | approve-plan <plan.json>');
