...
try {
  if (command === 'dashboard-close') console.log(JSON.stringify(await closeProgressDashboard()));
    ...
    else if (command === 'update') { const input = JSON.parse(await readFile(file, 'utf8')); if (!input.agentId || !['BLOCKED', 'DONE'].includes(input.status)) throw new Error('agent update requires agentId and final status BLOCKED or DONE'); console.log(JSON.stringify(progressMutationResult(await updateProgressStep(input), input.goalId, input.stepId))); }
    ...
    else throw new Error('usage: read | status | next <goal-id> | claim <agent-id> [goal-id] | update <update.json> | mode <goal-id> automatic | mode <goal-id> manual <visual-review|important-decision> | validate-plan <plan.json> | approve-plan <plan.json> | dashboard-close');
  }
