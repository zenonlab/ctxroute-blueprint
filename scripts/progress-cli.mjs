...
try {
  if (command === 'dashboard-close') console.log(JSON.stringify(await closeProgressDashboard()));
    ...
    else if (command === 'validate-plan') { const result = validatePlan(JSON.parse(await readFile(file, 'utf8')), current); console.log(JSON.stringify({ ok: result.ok, errors: result.errors })); if (!result.ok) process.exitCode = 1; }
