...
import { MANUAL_REASONS, archiveCompletedProgressGoals, ensureProgressView, validatePlan, approvePlan, progressStatus, progressNext, progressMutationResult, claimProgressTicket, updateProgressStep, setProgressMode } from './progress-core.mjs';
...
try {
  if (command === 'dashboard-close') console.log(JSON.stringify(await closeProgressDashboard()));
    ...
    else if (command === 'update') { const input = JSON.parse(await readFile(file, 'utf8')); if (!input.agentId || !['BLOCKED', 'DONE'].includes(input.status) || !Array.isArray(input.evidence) || !input.evidence.length) throw new Error('agent update requires agentId, final status BLOCKED or DONE, and evidence'); console.log(JSON.stringify(progressMutationResult(await updateProgressStep(input), input.goalId, input.stepId))); }
    ...
    else if (command === 'validate-plan') { const result = validatePlan(JSON.parse(await readFile(file, 'utf8')), current); console.log(JSON.stringify({ ok: result.ok, errors: result.errors, warnings: result.warnings })); if (!result.ok) process.exitCode = 1; }
    else if (command === 'approve-plan') { const input = JSON.parse(await readFile(file, 'utf8')); console.log(JSON.stringify(progressMutationResult(await approvePlan(input), input.goalId ?? input.id))); }
