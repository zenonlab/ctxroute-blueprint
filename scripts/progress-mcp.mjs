...
export function createProgressServer(root = process.cwd()) {
  ...
  server.registerTool('progress_read', { description: 'Read the full persistent checklist. Prefer progress_status or progress_next when full ticket text is unnecessary.', inputSchema: z.object({}) }, async () => result(await readProgress(root)));
