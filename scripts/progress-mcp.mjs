...
export function createProgressServer(root = process.cwd()) {
  ...
  server.registerResource('progress-full', PROGRESS_RESOURCE_URI, { title: 'Full Progress checklist', description: 'Complete JSON checklist for voluntary diagnostics. Prefer status, next, and claim tools for normal coordination.', mimeType: 'application/json' }, async uri => { await ready; return { contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(await readProgress(root)) }] }; });
  server.registerTool('progress_status', { description: 'Return a compact progress summary.', inputSchema: z.object({}) }, async () => { await ready; return result(progressStatus(await readProgress(root))); });
  server.registerTool('progress_validate_plan', { description: 'Validate a plan without writing files.', inputSchema: planSchema }, async plan => { const validation = validatePlan(plan, await readProgress(root)); return result({ ok: validation.ok, errors: validation.errors }); });
  server.registerTool('progress_approve_plan', { description: 'Validate and materialize an authorized plan. approved: true is the write flag, not a second confirmation. Returns a compact acknowledgement.', inputSchema: planSchema.extend({ approved: z.literal(true) }) }, async plan => result(progressMutationResult(await approvePlan(plan, root), plan.goalId ?? plan.id)));
  ...
  server.registerTool('progress_update_step', { description: 'Report DONE or BLOCKED once after work on a ticket already claimed by this agent. Returns a compact acknowledgement.', inputSchema: z.object({ goalId: z.string(), stepId: z.string(), agentId: z.string(), status: z.enum(['BLOCKED', 'DONE']), evidence: z.array(z.string()).optional() }) }, async input => result(progressMutationResult(await updateProgressStep(input, root), input.goalId, input.stepId)));
  ...
  server.registerTool('progress_set_mode', { description: 'Set automatic execution, or manual only with a visual-review or important-decision reason. Returns a compact acknowledgement.', inputSchema: modeSchema }, async ({ goalId, mode, reason }) => { const progress = await setProgressMode(goalId, mode, reason, root); return result({ ...progressMutationResult(progress, goalId), next: progressNext(progress, goalId) }); });
