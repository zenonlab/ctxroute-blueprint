...
export function createProgressServer(root = process.cwd()) {
  ...
  server.registerTool('progress_set_mode', { description: 'Set automatic execution or a manual pause for a visual review or important product/design decision. Returns a compact acknowledgement.', inputSchema: z.object({ goalId: z.string(), mode: z.enum(['automatic', 'manual']) }) }, async ({ goalId, mode }) => { const progress = await setProgressMode(goalId, mode, false, root); const goal = progress.goals.find(item => item.id === goalId); return result({ ok: true, goal: { id: goal.id, status: goal.status, executionMode: goal.executionMode }, next: progressNext(progress, goalId) }); });
