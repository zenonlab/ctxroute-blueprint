...
export function createProgressServer(root = process.cwd()) {
  ...
  server.registerTool('progress_claim_ticket', { description: 'Atomically claim one unassigned ticket for an agent. Call once before work.', inputSchema: z.object({ agentId: z.string(), goalId: z.string().optional() }) }, afterReady(async ({ agentId, goalId }) => result(await claimProgressTicket(agentId, goalId, root))));
  server.registerTool('progress_update_step', { description: 'Report DONE or BLOCKED once after work on a ticket already claimed by this agent. Returns a compact acknowledgement.', inputSchema: z.object({ goalId: z.string(), stepId: z.string(), agentId: z.string(), status: z.enum(['BLOCKED', 'DONE']), evidence: z.array(z.string()).optional() }) }, afterReady(async input => result(progressMutationResult(await updateProgressStep(input, root), input.goalId, input.stepId))));
  server.registerTool('progress_next', { description: 'Return at most three next steps for a goal.', inputSchema: goalIdSchema }, afterReady(async ({ goalId }) => result(progressNext(await readProgress(root), goalId))));
