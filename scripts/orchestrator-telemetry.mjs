...
export async function emitDecisionEvent(root, config, event, dependencies = {}) {
  ...
  const complete = {
    ...
    requested_mode: event.requested_mode ?? null,
    resolved_mode: event.resolved_mode ?? null,
    workflow: event.workflow ?? null,
    stage: event.stage ?? null,
    strategy: event.strategy ?? null,
    policy_digest: event.policy_digest ?? null,
    reinforcements: event.reinforcements ?? [],
    policy_justification: event.policy_justification ?? [],
