...
export function validateMcpInstallation(root = process.cwd()) {
  const errors = [];
  const expected = {
    'ctxroute-orchestrator': 'orchestrator:mcp',
  ...
  if (Object.keys(claude).sort().join(',') !== Object.keys(expected).sort().join(',')) errors.push('Claude must declare exactly ctxroute-orchestrator and code-review-graph.');
  ...
  if (!ORCHESTRATOR_TOOL_NAMES.every(name => name.startsWith('orchestrator_') || name === 'ctxroute_context_query')) errors.push('Orchestrator MCP responsibilities are mixed.');
  return { ok: errors.length === 0, servers: Object.keys(expected), orchestratorTools: [...ORCHESTRATOR_TOOL_NAMES].sort(), contextProviders: ['ctxroute@2.0.0', 'code-review-graph@2.3.8'], errors };
}
