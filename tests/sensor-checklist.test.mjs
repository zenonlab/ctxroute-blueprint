...
test('sensor checklist is read-only and passes in JSON and terminal formats', () => {
  ...
  assert.deepEqual(result.optionalParsers.map(item => item.id), ['ruby-ast', 'php-ast']);
  assert.equal(result.checks.find(item => item.name === 'optional-parsers').status, 'PASS');
  ...
  assert.deepEqual(result.adapters.find(item => item.id === 'lexical-source').filenames, ['Dockerfile', 'Makefile', 'Justfile', 'Gemfile', 'Rakefile', 'config.ru']);
