...
test('sensor checklist is read-only and passes in JSON and terminal formats', () => {
  ...
  assert.deepEqual(result.optionalParsers.map(item => item.id), ['ruby-ast', 'erb-ast']);
  assert.equal(result.optionalParsers.every(item => item.available && item.grammar !== null), true);
  assert.equal(result.checks.find(item => item.name === 'ruby-grammars').status, 'PASS');
