...
test('governance policy is complete and non-overlapping', () => {
  ...
  assert.equal(decide('routing.change', { approved: true, decisionReceipt: { receipt_id: 'receipt-one', selection: 'approve' } }).allowed, true);
  assert.equal(decide('memory.observe').allowed, true);
  assert.equal(decide('memory.resolve', { authority: 'orchestrator' }).allowed, true);
  assert.equal(decide('repository.repair').allowed, false);
