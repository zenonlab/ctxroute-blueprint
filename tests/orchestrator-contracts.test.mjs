...
const state = { schemaVersion: 2, revision: 1, mode: 'SWARM_ON', telemetry_sequence: 0, goals: [{ goal_id: 'goal-one', title: 'Implement contracts', status: 'ACTIVE', missions: [missionRecord] }], skills: [{ skill_id: 'blueprint-audit', version: '2.0.0', path: '.agents/skills/blueprint-audit', artifact_digest: digest, validation_receipt: receipt, audit_id: 'audit-one', registered_revision: 1 }], audits: [audit], transactions: [{ operation_id: 'operation-one', digest, action: 'mission.prepare', status: 'COMPLETED', start_revision: 0, end_revision: 1, result: 'CREATED', cause: null }], worktree_operations: [worktreeOperation] };
...
test('all public orchestrator schemas compile once and accept their canonical examples', () => {
  assert.equal(listOrchestratorContracts().length, 11);
