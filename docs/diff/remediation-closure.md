...
scope and its executable evidence. Scope labels describe the delivered change;
the release handoff records final hashes and command results. `PASS` means the cited behavior is exercised
...
| --- | --- | --- | --- |
| R-01 | Transactional core | Contract/replay cases in `tests/orchestrator-core.test.mjs` | PASS: any full-payload difference rejects before effects; identical replay converges |
| R-02 | Strict contracts | `npm run orchestrator:contracts` and state load fixtures | PASS: malformed, unknown-field, secret-bearing, corrupt, and incompatible state rejects |
...
| R-06 | Transactional core | live/dead/token-changed lock and timeout fixtures | PASS: live locks remain; only unchanged dead stale owners are reclaimed |
| R-07 | Governance policy and evidence | `npm run validate:docs -- --all` and ADR-0027 | LIMITATION ACCEPTED: cooperative local policy boundary; no hostile-process sandbox claim |
...
| R-09 | Strict contracts | exhaustive goal and mission transition table | PASS: illegal and terminal-state transitions reject |
| R-10 | Mission routing | environment/state/default plus direct/coordinated/auto cases | PASS: precedence and `mode_source` are exact; environment `SWARM_OFF` is direct |
...
| R-12 | Strict contracts | deep key/value/diagnostic secret matrix and telemetry scan | PASS: sensitive content rejects or is redacted before persistence |
...
| R-15 | Governance policy and evidence | `npm run validate:docs -- --all` and `npm run check:coherence` | PASS: active docs expose stdio/CLI/local files only and describe the exact CI matrix |
| R-16 | Governance policy and evidence | `npm run validate:decisions` and duplicate-ID negative test | PASS: numeric IDs are unique; supersession targets must exist; historical dashboard ADR is archived |
| R-17 | Governance policy and evidence | `tests/ci-security.test.mjs` and `npm run check:coherence` | PASS: `fail-on-risk: high` and the `CRG risk gate` name are enforced |
...
| R-19 | Transactional core | injected temp/write/fsync/rename failures and reload after every crash point | PASS with platform limitation: file and supported directory sync occur; unsupported directory sync is explicit |
