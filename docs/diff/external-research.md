...
| [JSON Schema basics](https://json-schema.org/understanding-json-schema/basics) — JSON Schema project | `$schema` identifies the dialect and `$id` gives a stable schema identity. | Documented fact; recommendation | High. Use stable v1 schema IDs alongside the existing business `schemaVersion`. |
